...
const { execute: createCategory, loading: posting } =
  serviceTagApi.post({
    ...
    onSuccess: () => {
      router.push({ name: 'admin.serviceTag.index' });
    }
  });
  ...
  <div class="q-pa-md q-pa-sm-lg q-pa-md-xl">
    <div class="row items-center q-gutter-sm q-mb-xl">
      <q-icon name="keyboard_backspace" size="30px" style="cursor: pointer" @click="() => {
        ...
        " />
      <span class="text-h6"> Add Service Tags </span>
    </div>
    <q-form @submit="submit" @validation-error="srollToView">
      <div class="q-gutter-y-sm">
        ...
        <div class="row q-col-gutter-md">
          <div class="col-12 q-mb-xl">
            <div class="q-py-xs" style="font-weight: 500;">Tag Thumbnail</div>
            <ImageInput name="image" @image="(f) => { form.image = f as unknown as null }" style="max-width: 15rem;" />
          </div>
          <q-input outlined v-model="form.category.name" label="Tag Name" class="col-12 col-sm-6 col-md-3" :rules="[
      ...
      <div>
        ...
        <div class="row q-col-gutter-lg">
          <q-input outlined v-model="form.category.shortDesc" label="Tag Short Description" class="col-12 " />
          <q-input type="textarea" outlined v-model="form.category.longDesc" label="Tag Long Description"
            class="col-12" />
        </div>
      </div>
      <div class="q-pb-lg">
