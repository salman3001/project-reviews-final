...
const { onRequest, loading, rows } = onTableRequest(serviceSubCategoryApi, pagination)
    ...
    <div class="colomn q-gutter-y-lg" style="width: 100%">
      <div class="row justify-between q-gutter-y-sm">
        ...
        <div class="row q-gutter-sm">
          ...
          <ImportExcel type="service-subcategory" />
          <ExportExcel type="service-subcategory" />
          <q-btn color="primary" @click="() => {
            router.push({
              name: 'admin.serviceSubcategory.create',
            })
          }
            ">+ Add Subcategory</q-btn>
        </div>
      </div>

      <q-table ref="tableRef" flat bordered title="Service Subcategories" :loading="loading" :rows="rows"
        :columns="colomns" class="zebra-table" v-model:pagination="pagination" :filter="filter" @request="onRequest"
        row-key="id">
                ...
                <q-list dense>
                  ...
                  <q-item clickable v-close-popup @click="
                    modal.togel('deleteRecord', {
                      url: '/service-subcategory/' + props.row.id,
                      tableRef,
