...
const { data: roles, loading: rolesLoading, trigger: getRoles } = useGet('roles', {})
...
const colomns: QTableProps['columns'] = [
  ...
  { name: 'role', field: (row: any) => row?.role?.name, label: 'Role', align: 'center' },
    ...
    <div class="colomn q-gutter-y-lg" style="width: 100%">
      <div class="row justify-between q-gutter-y-sm">
        ...
        <div class="row q-gutter-sm">
          <q-select v-model="search" v-if="!rolesLoading" dense options-dense emit-value outlined :options="[{ label: 'All', value: null }, ...roles.map((r: any) => ({
            label: r.name,
            value: r.id,
          }))]" label="Role" class="col-auto" style="min-width: 8rem" />
          <q-select outlined dense options-dense emit-value map-options v-model="status"
            :options="[{ label: 'All', value: null }, { label: 'Active', value: 1 }, { label: 'Inactive', value: 0 }]"
          ...
          <q-btn-dropdown outline label="Export" style="border: 1px solid lightgray;">
            <q-list dense>
              <q-item clickable v-close-popup @click="exportCSV(colomns, data)">
                <q-item-section>
                  <q-item-label> <q-icon name="receipt_long" /> Export CSV</q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
          </q-btn-dropdown>
          <q-btn color="primary" @click="() => { router.push({ name: 'admin.adminUsers.create' }) }">+ Add User</q-btn>
        </div>
      </div>
      <q-table ref="tableRef" flat bordered title="Admin Users" :loading="loading" :rows="data" :columns="colomns"
        ...
        <template v-slot:body-cell-first_name="props">
          <q-td :props="props" class="row q-gutter-xs items-center">
            <q-avatar size="30px">
              <img :src="uploads + props.row?.avatar?.url"
                :style="{ border: props.row.is_active === 1 ? '2px green solid' : '2px red solid' }">
            </q-avatar>
            <div>
          ...
          <q-td :props="props">
            <div class="">
              <q-btn-dropdown size="sm" color="primary" label="Options">
                <q-list dense>
                  <q-item clickable v-close-popup @click="modal.togel('changeRole', {
                    roles: roles, id: props.row.id, selectedRole: props?.row?.role?.id, tableRef
                  })">
                    <q-item-section>
                      <q-item-label> <q-icon name="published_with_changes" /> Change Role</q-item-label>
                    </q-item-section>
                  </q-item>

                  <q-item clickable v-close-popup @click="modal.togel('changeAdminStatus', {
                    id: props.row.id, tableRef
                  })">
                    <q-item-section>
                      <q-item-label> <q-icon name="highlight_off" />Ban User</q-item-label>
                    </q-item-section>
                  </q-item>

                  <q-item clickable v-close-popup>
                    <q-item-section>
                      <q-item-label> <q-icon name="edit" /> Edit User</q-item-label>

                    </q-item-section>
                  </q-item>
                  <q-item clickable v-close-popup @click="modal.togel('deleteRecord', {
                    id: props.row.id, tableRef, title: 'Delete User?'
                  })">
                    <q-item-section>
                      <q-item-label> <q-icon name="delete" /> Delete User</q-item-label>
                    </q-item-section>
                  </q-item>
                  <q-item clickable v-close-popup @click="() => {
                    ...
                    <q-item-section>
                      <q-item-label> <q-icon name="rowing" /> Activity log</q-item-label>
                    </q-item-section>
                  </q-item>
                </q-list>
              </q-btn-dropdown>
