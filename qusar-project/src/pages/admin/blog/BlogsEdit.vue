...
BlogApi.show(route.params.id as string, {
  ...
  form.value.blogCategoryId = (data.value as any)?.category?.id
  ...
  form.value.isPublished = (data.value as any)?.is_published == 1 ? true : false
})

const { execute: createBlog, loading: posting } =
  BlogApi.put(route.params.id as string, form.value, {
    headers: {
      ...
      <div class="q-gutter-y-md">
        <div class="row q-col-gutter-md">
          ...
          <div style="max-width: 250px;">
            <ImageInput name="image" width="250px" :url="blog?.image?.url ? uploads + blog?.image?.url : undefined" />
          </div>
