...
templateApi.show(route.params.id as string).then(({ data }) => {
  ...
  form.value.template.desc = (data.value as any)?.desc || ''
  form.value.template.content = (data.value as any)?.content || ''
})
  ...
  <div class="q-pa-md q-pa-sm-lg q-pa-md-xl">
    <div class="row items-center q-gutter-sm q-mb-xl">
      <q-icon name="keyboard_backspace" size="30px" style="cursor: pointer" @click="() => {
        ...
        " />
      <span class="text-h6"> Edit Template </span>
    </div>
      ...
      <div class="q-gutter-y-sm">
        <div class="row q-col-gutter-md">
          <div class="col-12 q-mb-xl">
            ...
            <ImageInput name="image" @image="(f) => { form.thumbnail = f as unknown as null }" style="max-width: 15rem;"
              :url="template?.thumbnail?.url ? uploads + template?.thumbnail?.url : undefined" />
          </div>
