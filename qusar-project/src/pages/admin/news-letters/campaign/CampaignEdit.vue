...
campaignApi.show(route.params.id as string, {
  ...
  form.value.campaign.campaignTypeId = (data.value as any).campaign_type_id || ''
  form.value.campaign.deliveryDateTime = (data.value as any).delivery_date_time ? formatDate((data.value as any).delivery_date_time, 'DD/MM/YYYY HH:mm') : ''
  form.value.campaign.fromEmail = (data.value as any).from_email || ''
  form.value.campaign.fromName = (data.value as any).from_name || ''
  form.value.campaign.name = (data.value as any).name || ''
  form.value.campaign.replyTo = (data.value as any).reply_to || ''
  form.value.campaign.status = (data.value as any).status == 1 ? true : false
  form.value.campaign.subject = (data.value as any).subject || ''

  form.value.interests = (data.value as any).interests ? (data.value as any).interests.map((i: any) => i?.id) : []
  form.value.templateId = (data.value as any)?.template?.id || ''

  template.value = (data.value as any)?.template || null
})
  ...
  <div class="q-pa-md">
    <div class="row items-center q-gutter-sm">
      <q-icon name="keyboard_backspace" size="30px" style="cursor: pointer" @click="() => {
        ...
        " />
      <span class="text-h6"> Edit Campaign </span>
    </div>
    <div>
