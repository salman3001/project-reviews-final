...
const continent = ref<null | Record<string, any>>(null);

ContinentsApi.show(modal.meta?.id).then(({ data }) => {
  continent.value = data.value;
...
<template>
  <q-card style="width: 100%">
    <q-toolbar style="background-color: #ebeae4">
      <q-toolbar-title><span class="text-weight-bold">Edit Continent</span></q-toolbar-title>
