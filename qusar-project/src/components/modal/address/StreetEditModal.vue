...
const street = ref<null | Record<string, any>>(null);

StreetApi.show(modal.meta?.id).then(({ data }) => {
  street.value = data.value;
...
<template>
  <q-card style="width: 100%">
    <q-toolbar style="background-color: #ebeae4">
      <q-toolbar-title><span class="text-weight-bold">Edit street</span></q-toolbar-title>
